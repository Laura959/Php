...
if (!file_exists("Antras_dvitaskiai.txt")) { // patikrina, ar egzistuoja failas
    exit ("No messages!");
    ...
    $fp = fopen ("Antras_dvitaskiai.txt", "r+");
    $n = file("Antras_dvitaskiai.txt");
    fclose($fp);
    ...
    if (!file_exists("Tritaskiai/2.txt")) { // patikrina, ar egzistuoja failas
        exit ("No messages!");
    } else { // jei failas yra, jis yra skaitomas, t.y. atidaromas (fopen) ir tekstas is failo nuskaitomas ir perkeliamas i masyva, ir uzdaromas (fclose) "file"
        $fp = fopen ("Tritaskiai/2.txt", "r+");
        $n = file("Tritaskiai/2.txt");
        fclose($fp);
